    ...
    <el-col :span="12">
      <div class="left-action-box">
        <div class="collapse-box">
          <el-button @click="turnLeftCollapse" v-if="storeDisplay.showMenu" text :icon="ElIconFold"
                     class="fold-btn"></el-button>
      ...
      <div class="header-action-box">
        <template v-if="storeDisplay.currentPage === 'view'">
          <el-tooltip v-if="storePage.pageAuth.canEdit === 1" content="编辑文档">
            <el-button class="hover-button" @click="editWiki" text>
              <IconParkEdit size="18"/>
            </el-button>
          </el-tooltip>
          <el-tooltip content="文档沟通">
            <el-button class="hover-button" @click="showCommentWiki" text>
              <IconParkCommunication size="18"/>
            </el-button>
          </el-tooltip>
          <UserMessagePopover/>
...
import {useStorePageData} from '@/stores/pageData.js'
...
const deleteWikiPage = () => {
  ElMessageBox.confirm('确定要删除此页面及其所有子页面吗？', '提示', {
    ...
    pageDelete({pageId: storePage.pageInfo.id}).then(() => {
