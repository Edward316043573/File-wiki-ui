  ...
  <el-dialog title="设置为模板" v-model="newTemplateDialogVisible" width="600px" :close-on-click-modal="false">
    <el-form label-width="100px" :model="templateNewForm">
      ...
      <el-form-item label="是否公开">
        <el-switch v-model="templateNewForm.shareStatus" inactive-text="个人模板" :inactive-value="0" active-text="公共模板"
...
import {addTemplate, useTemplate, getTemplate, getTags} from '@/api/page'
...
const onNewTemplateSubmit = () => {
  addTemplate(templateNewForm.value).then((json) => {
...
const filterByOpen = () => {
  getTags({open: open.value}).then((json) => {
